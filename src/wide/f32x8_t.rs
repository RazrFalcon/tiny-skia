    ...
    pub fn floor(self) -> Self {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "simd128"))] {
                ...
                Self(unsafe { vrndmq_f32(self.0) }, unsafe { vrndmq_f32(self.1) })
            } else {
    ...
    pub fn cmp_eq(self, rhs: Self) -> Self {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(
                    cast(unsafe { vceqq_f32(self.0, rhs.0) }),
                    cast(unsafe { vceqq_f32(self.1, rhs.1) }),
                )
            } else {
    ...
    pub fn cmp_ge(self, rhs: Self) -> Self {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(
                    cast(unsafe { vcgeq_f32(self.0, rhs.0) }),
                    cast(unsafe { vcgeq_f32(self.1, rhs.1) }),
                )
            } else {
    ...
    pub fn cmp_gt(self, rhs: Self) -> Self {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(
                    cast(unsafe { vcgtq_f32(self.0, rhs.0) }),
                    cast(unsafe { vcgtq_f32(self.1, rhs.1) }),
                )
            } else {
    ...
    pub fn cmp_ne(self, rhs: Self) -> Self {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(
                    cast(unsafe { vmvnq_u32(vceqq_f32(self.0, rhs.0)) }),
                    cast(unsafe { vmvnq_u32(vceqq_f32(self.1, rhs.1)) }),
                )
            } else {
    ...
    pub fn cmp_le(self, rhs: Self) -> Self {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(
                    cast(unsafe { vcleq_f32(self.0, rhs.0) }),
                    cast(unsafe { vcleq_f32(self.1, rhs.1) }),
                )
            } else {
    ...
    pub fn cmp_lt(self, rhs: Self) -> Self {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(
                    cast(unsafe { vcltq_f32(self.0, rhs.0) }),
                    cast(unsafe { vcltq_f32(self.1, rhs.1) }),
                )
            } else {
    ...
    pub fn blend(self, t: Self, f: Self) -> Self {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                unsafe fn vbslq_u32_rs(a: float32x4_t, t: float32x4_t, f: float32x4_t) -> float32x4_t {
                    cast(vbslq_u32(
                        cast(a),
                        cast(t),
                        cast(f),
                ...
                Self(
                    unsafe { vbslq_u32_rs(self.0, t.0, f.0) },
                    unsafe { vbslq_u32_rs(self.1, t.1, f.1) },
                )
            } else {
    ...
    pub fn abs(self) -> Self {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "simd128"))] {
                ...
                Self(unsafe { vabsq_f32(self.0) }, unsafe { vabsq_f32(self.1) })
            } else {
    ...
    pub fn max(self, rhs: Self) -> Self {
        ...
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(unsafe { vmaxq_f32(self.0, rhs.0) }, unsafe { vmaxq_f32(self.1, rhs.1) })
            } else {
                Self([
                    ...
                    self.0[4].faster_max(rhs.0[4]),
                    self.0[5].faster_max(rhs.0[5]),
                    self.0[6].faster_max(rhs.0[6]),
                    self.0[7].faster_max(rhs.0[7]),
                ])
            }
        }
    }
    ...
    pub fn min(self, rhs: Self) -> Self {
        ...
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(unsafe { vminq_f32(self.0, rhs.0) }, unsafe { vminq_f32(self.1, rhs.1) })
            } else {
                Self([
                    ...
                    self.0[4].faster_min(rhs.0[4]),
                    self.0[5].faster_min(rhs.0[5]),
                    self.0[6].faster_min(rhs.0[6]),
                    self.0[7].faster_min(rhs.0[7]),
                ])
            }
        }
    }
    ...
    pub fn round(self) -> Self {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(unsafe { vrndnq_f32(self.0) }, unsafe { vrndnq_f32(self.1) })
            } else {
    ...
    pub fn round_int(self) -> i32x8 {
        ...
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                i32x8(unsafe { vcvtnq_s32_f32(self.0) }, unsafe { vcvtnq_s32_f32(self.1) })
            } else {
    ...
    pub fn trunc_int(self) -> i32x8 {
        ...
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                i32x8(unsafe { vcvtq_s32_f32(self.0) }, unsafe { vcvtq_s32_f32(self.1) })
            } else {
    ...
    pub fn sqrt(self) -> Self {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(unsafe { vsqrtq_f32(self.0) }, unsafe { vsqrtq_f32(self.1) })
            } else {
                Self::from([
                    self.0[0].sqrt(),
                    self.0[1].sqrt(),
    ...
    fn add(self, rhs: Self) -> Self::Output {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(unsafe { vaddq_f32(self.0, rhs.0) }, unsafe { vaddq_f32(self.1, rhs.1) })
            } else {
    ...
    fn sub(self, rhs: Self) -> Self::Output {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(unsafe { vsubq_f32(self.0, rhs.0) }, unsafe { vsubq_f32(self.1, rhs.1) })
            } else {
    ...
    fn mul(self, rhs: Self) -> Self::Output {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(unsafe { vmulq_f32(self.0, rhs.0) }, unsafe { vmulq_f32(self.1, rhs.1) })
            } else {
    ...
    fn div(self, rhs: Self) -> Self::Output {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(unsafe { vdivq_f32(self.0, rhs.0) }, unsafe { vdivq_f32(self.1, rhs.1) })
            } else {
    ...
    fn not(self) -> Self {
        cfg_if::cfg_if! {
            if #[cfg(all(feature = "simd", target_feature = "avx"))] {
                ...
                Self(
                    cast(unsafe { vmvnq_u32(cast(self.0)) }),
                    cast(unsafe { vmvnq_u32(cast(self.1)) }),
                )
            } else {
